...
从项目根目录执行命令。目标是用最小充分的测试证明本次用户可见行为；测试数量少本身不是目标。遵循根 `AGENTS.md` 的按改动范围验证要求；Rust 改动选择相关 crate/测试名，不默认运行 workspace 全量。
...
- 不默认运行裸 `pnpm test:e2e`，也不在专项通过后追加一次全量“保险”。只有用户明确要求全量时才运行全套；全局启动/共享配置改动仍先选择相关集合，说明未覆盖风险。先评估所选套件是否需要真实账号和外部服务。
