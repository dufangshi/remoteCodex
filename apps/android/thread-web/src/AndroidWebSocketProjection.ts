...
function updateTurnStatus(
  ...
  return {
    ...
    pendingSteers: ensured.pendingSteers.filter((steer) => steer.turnId !== turnId),
  };
}
...
function appendOutputDelta(
  ...
  return updateTurn(detail, payload.turnId, (turn) => {
    ...
    const nextText = text.endsWith(payload.delta) ? text : `${text}${payload.delta}`;
    const nextItem: ThreadHistoryItemDto = existing
      ? { ...existing, text: nextText, status: existing.status ?? 'running' }
      : {
          id: payload.itemId,
    ...
    return { ...turn, items: upsertById(turn.items, nextItem) };
  });
}
...
function isProjectableThreadEvent(
  ...
  switch (event.type) {
    ...
    case 'thread.item.started':
    case 'thread.item.completed':
      return typeof event.payload.turnId === 'string' && isThreadHistoryItem(event.payload.item);
    case 'thread.output.delta':
      return (
    ...
    case 'thread.plan.updated':
      return (
        typeof event.payload.turnId === 'string' &&
        (typeof event.payload.explanation === 'string' || event.payload.explanation === null) &&
        Array.isArray(event.payload.plan)
      );
    case 'thread.request.created':
      return isRecord(event.payload.request) && typeof event.payload.request.id === 'string';
    case 'thread.request.resolved':
      return typeof event.payload.requestId === 'string';
    case 'thread.turn.token.updated':
      return typeof event.payload.turnId === 'string' && isRecord(event.payload.tokenUsage);
    default:
      return false;
  }
}
