...
describe('supervisor api', () => {
  ...
  it('configures log redaction for credentials', () => {
  ...
  it('keeps WebView CORS disabled unless explicitly enabled', async () => {
    const response = await app.inject({
      method: 'GET',
      url: '/api/version',
      headers: {
  ...
  it('allows explicit WebView API requests', async () => {
    await app.close();
    app = buildTestApp(fakeCodexManager, {
      env: {
    ...
    expect(androidResponse.headers['access-control-allow-origin']).toBe(
  ...
  it('rejects deleting a workspace without confirmation payload', async () => {
    const workspaceResponse = await app.inject({
      method: 'POST',
      url: '/api/workspaces',
      payload: {
    ...
    expect(listWorkspacesResponse.statusCode).toBe(200);
    expect(listWorkspacesResponse.json()).toHaveLength(1);
  });
