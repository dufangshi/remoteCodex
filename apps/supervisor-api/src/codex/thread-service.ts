...
function addTurnTokenBreakdowns(
  left: ThreadTurnTokenUsageBreakdown,
  right: ThreadTurnTokenUsageBreakdown,
...
function buildThreadTurnTokenUsage(
  ...
  return {
    total: previous ? addTurnTokenBreakdowns(previous.total, last) : last,
    last,
    modelContextWindow:
...
function parseThreadTurnTokenUsageJson(
  ...
  try {
    return parseThreadTurnTokenUsage(JSON.parse(value) as Record<string, unknown>);
  } catch {
    return null;
  }
}
...
function buildTurnDto(
  ...
  return {
    ...
    tokenUsage: parseThreadTurnTokenUsageJson(metadata?.tokenUsageJson),
  };
}
  ...
  private async handleNotification(event: CodexServerEvent) {
    switch (event.method) {
      ...
      case 'thread/tokenUsage/updated': {
        ...
        const previousTurnTokenUsage = parseThreadTurnTokenUsageJson(
          getThreadTurnMetadataByThreadAndTurnId(
            this.db,
            record.id,
            params.turnId,
        ...
        const turnTokenUsage = buildThreadTurnTokenUsage(
          params.tokenUsage,
          previousTurnTokenUsage,
        );
        if (turnTokenUsage) {
          upsertThreadTurnMetadata(this.db, {
            threadId: record.id,
            turnId: params.turnId,
            tokenUsageJson: JSON.stringify(turnTokenUsage),
          });
