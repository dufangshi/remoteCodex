    ...
    const shellSocket = connectShellSocket({
      onConnected: () => {
        ...
        attachTimeoutRef.current = window.setTimeout(() => {
          ...
          if (socketRef.current?.socket !== shellSocket.socket || viewerIdRef.current) {
            return;
          }
...
>(function ThreadShellPanel(
  ...
  useEffect(() => {
    ...
    const parsed = Number.parseFloat(storedRatio);
  ...
  const handleSubmitRenameShell = useCallback(async () => {
    ...
    try {
      ...
      const updated = await updateShell(renamingShellId, {
      ...
      setError(caught instanceof ApiError ? caught.payload.message : 'Unable to rename shell.');
    } finally {
      setBusy(false);
    }
  ...
  const handleCreateShell = useCallback(
    async (paneId: ShellPaneId = activePaneId) => {
      ...
      try {
        ...
        setError(
          caught instanceof ApiError ? caught.payload.message : 'Unable to create shell.',
        );
      } finally {
    ...
    [activePaneId, setPaneShell, splitMode, threadId],
  );
  ...
  const handleTerminateShell = useCallback(
    async (shellId: string = activeShell?.id ?? '') => {
      ...
      try {
        ...
        setError(
          caught instanceof ApiError ? caught.payload.message : 'Unable to terminate shell.',
        );
      } finally {
        setBusy(false);
      }
    },
    [activeShell?.id, loadShellState],
  );
  ...
  const persistSplitRatio = useCallback(
    (nextRatio: number) => {
      ...
      window.localStorage.setItem(
        shellLayoutStorageKey(threadId),
        String(clampPaneRatio(nextRatio)),
      );
    },
    [threadId],
  );
