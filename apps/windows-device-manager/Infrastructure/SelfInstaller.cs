...
internal static class SelfInstaller
{
    public static void InstallAndRestart(string installedPath, IReadOnlyList<string> originalArguments)
    {
        ...
        StopExistingInstalledInstance(installedPath);
    ...
    private static void StopExistingInstalledInstance(string installedPath)
    {
        foreach (var process in Process.GetProcessesByName("RemoteCodex.DeviceManager"))
        {
            using (process)
            {
                ...
                {
                    throw new TimeoutException("The previous Device Manager did not exit during update.");
                }
            }
        }
    }
