...
{
    public static async Task<int> RunNodeProvisioningAsync()
    {
        try
        {
            ...
            var nodePath = await provisioner.ProvisionNodeForTestAsync(timeout.Token);
        ...
        catch (Exception exception)
        {
            try
            {
                new AppLogger().Error("Node.js provisioning self-test failed", exception);
            }
            catch
            {
                // Preserve the self-test result when logging is unavailable.
            }
            return 1;
        }
    }

    public static async Task<int> RunRuntimeProvisioningAsync(string nodePath)
    {
        try
        {
            ...
            var runtime = await provisioner.ProvisionBundledRuntimeForTestAsync(
                nodePath,
                timeout.Token);
            if (!runtime.IsUsable
                || runtime.AcpCommandPath is not { } acpCommand
                || runtime.NativeBinaryPath is not { } nativeBinary)
            {
                return 1;
            }
            var launcher = await runner.RunAsync(
                nodePath,
                [runtime.RemoteCodexEntryPath, "--version"],
                TimeSpan.FromSeconds(30),
                cancellationToken: timeout.Token);
            var adapter = await runner.RunAsync(
                ...
                TimeSpan.FromSeconds(30),
                cancellationToken: timeout.Token);
            var native = await runner.RunAsync(
                nativeBinary,
                ["version"],
                TimeSpan.FromSeconds(30),
                cancellationToken: timeout.Token);
            return launcher.Success
                && launcher.StandardOutput.Trim() == ProductManifest.RemoteCodexVersion
                && adapter.Success
                && adapter.StandardOutput.Contains(ProductManifest.CodexAcpVersion, StringComparison.Ordinal)
                && native.Success
                && native.StandardOutput.Trim() == ProductManifest.RemoteCodexVersion
                    ? 0
                    : 1;
        }
        catch (Exception exception)
        {
            try
            {
                new AppLogger().Error("Runtime provisioning self-test failed", exception);
            }
            catch
            {
                // Preserve the self-test result when logging is unavailable.
    ...
    public static async Task<int> RunAsync()
    {
        ...
        try
        {
            ...
            var where = await runner.RunAsync(
                ...
                ["cmd.exe"],
                TimeSpan.FromSeconds(10));
            var managedPrefix = Path.Combine(testDirectory, "managed-runtime");
            var remoteCodexEntry = Path.Combine(
                managedPrefix,
                "node_modules",
                "remote-codex",
                "bin",
                "remote-codex.mjs");
            var fakeNode = Path.Combine(testDirectory, "node.exe");
            var fakeCodex = Path.Combine(testDirectory, "codex.exe");
            var fakeNative = Path.Combine(managedPrefix, "remote-codex-native.exe");
            Directory.CreateDirectory(Path.GetDirectoryName(remoteCodexEntry)!);
            await File.WriteAllTextAsync(remoteCodexEntry, string.Empty);
            await File.WriteAllTextAsync(fakeNode, string.Empty);
            await File.WriteAllTextAsync(fakeCodex, string.Empty);
            await File.WriteAllTextAsync(fakeNative, string.Empty);
            await File.WriteAllTextAsync(Path.Combine(managedPrefix, "codex-acp.cmd"), string.Empty);
            ...
            var processChecksPassed = shim.Success
                ...
                && where.Success
                && runtime.IsUsable
                && string.Equals(runtime.ManagedNpmPrefix, managedPrefix, StringComparison.OrdinalIgnoreCase);
