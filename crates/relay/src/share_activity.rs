...
mod tests {
    use super::*;
    #[test]
    fn visits_persist_deduplicate_and_stay_scoped() {
        let path = std::env::temp_dir().join(format!("share-activity-{}.sqlite", Uuid::new_v4()));
        ...
        drop(conn);
        let conn = Connection::open(&path).unwrap();
        let (at, username, events) = fields(&conn, "share", "s");
        assert_eq!(at, "2026-09-06T10:00:00Z");
        assert_eq!(username, "friend");
        assert_eq!(events.len(), 1);
        assert_eq!(events[0]["shareId"], "s");
        assert!(events[0].get("grantId").is_none());
        assert!(fields(&conn, "share", "other").2.is_empty());
        assert_eq!(fields(&conn, "grant", "s").2[0]["kind"], "open_device");
        drop(conn);
        std::fs::remove_file(path).unwrap();
    }
    #[test]
    fn visits_ignore_background_endpoints() {
        assert_eq!(
            request_kind(&Method::GET, "/api/threads/t?turnLimit=3"),
            Some("open_thread")
        );
        assert_eq!(request_kind(&Method::GET, "/api/threads/t/turns"), None);
        assert_eq!(request_kind(&Method::GET, "/api/status"), None);
        assert_eq!(
            request_kind(&Method::POST, "/api/threads/t/prompt"),
            Some("send_prompt")
        );
    }
}
