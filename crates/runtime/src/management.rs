...
mod tests {
    use super::*;
    #[test]
    fn installation_uses_the_executable_prefix_not_path_npm() {
        let root = tempfile::tempdir().unwrap();
        let prefix = root.path().join("node-v24");
        for dir in [
            "bin",
            "lib/node_modules/npm/bin",
            "lib/node_modules/@openai/codex/bin",
        ] {
        ...
        for file in [
            ...
            std::fs::write(prefix.join(file), "").unwrap();
        }
        std::fs::write(
        ...
        if !cfg!(windows) {
            assert_eq!(found.manager, "npm");
            assert!(found
                .update
                .contains(&text_path(&prefix.canonicalize().unwrap())));
            assert_eq!(found.version.as_deref(), Some("1.2.3"));
        }
        let app = installation(
            root.path().join("Codex.app/Contents/Resources/codex"),
            "codex",
        );
        assert_eq!(app.manager, "app");
        assert!(app.update.is_empty());
    }
    #[test]
    fn windows_npm_shim_resolves_its_own_package_and_rejects_traversal() {
        let root = tempfile::tempdir().unwrap();
        let target = root.path().join("node_modules/@openai/codex/bin/codex.js");
        ...
        .unwrap();
        assert_eq!(npm_shim_target(&shim), Some(target.canonicalize().unwrap()));
        std::fs::write(&shim, r#""%dp0%\node_modules\..\outside.js""#).unwrap();
        assert!(npm_shim_target(&shim).is_none());
    }
    #[test]
    fn brew_detection_is_scoped_and_unknown_installs_are_readonly() {
        let root = tempfile::tempdir().unwrap();
        std::fs::create_dir_all(root.path().join("bin")).unwrap();
        std::fs::write(root.path().join("bin/brew"), "").unwrap();
        let found = installation(root.path().join("Caskroom/codex/1.2/codex"), "codex");
        assert_eq!(&found.update[1..], &["upgrade", "--cask", "codex"]);
        assert!(installation(root.path().join("custom/codex"), "codex")
            .update
            .is_empty());
    }
}
