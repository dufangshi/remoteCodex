...
async fn context_window_restores_native_history_and_persists_acp_without_billing_tokens() {
    ...
    supervisor
        ...
        .with(|conn| {
            conn.execute(
                "UPDATE thread_turns SET token_usage_json=?1 WHERE id='live-turn'",
                params![json!({
    ...
    for (used, size) in [(40000, 200000), (60000, 256000), (100000, 1000000)] {
        ...
        let usage = supervisor
            .get_thread_detail(&thread_id, None)
            .await
            .unwrap()
            .thread
            .context_usage
    ...
    emit(
        ...
        &thread_id,
        "thread.context.updated",
        json!({"contextUsage":{"availability":"unavailable"}}),
    );
    assert_eq!(
        supervisor
            .get_thread(&thread_id)
