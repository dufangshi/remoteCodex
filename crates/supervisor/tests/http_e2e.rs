...
async fn linked_files_preview_external_bytes_without_relaxing_workspace_paths() {
    ...
    let workspace = json(
        ...
        client
            ...
            .json(&json!({"absPath":workspace_path,"label":"Links"})),
    )
    .await;
    let thread = json(
        &client,
        client
            .post(format!("{base}/api/threads/start"))
            .json(&json!({"workspaceId":workspace["id"],"provider":"codex","model":"fake"})),
    )
    .await;
    ...
    let stat = json(
        &client,
        client
            .get(format!("{base}/api/threads/{tid}/linked-files/stat"))
            .query(&[("path", external)]),
    )
    .await;
    ...
    let raw = client
        .get(format!("{base}/api/threads/{tid}/linked-files/raw"))
        .query(&[("path", external)])
        .send()
        .await
        .unwrap();
    ...
    let preview = json(
        &client,
        client
            .get(format!("{base}/api/threads/{tid}/linked-files/preview"))
            .query(&[
    ...
    for path in [
        ...
        "/nonexistent-generated-test.png",
    ] {
        assert_eq!(
            client
                .get(format!("{base}/api/threads/{tid}/linked-files/stat"))
    ...
    assert_eq!(
        client
            .get(format!(
                "http://127.0.0.1:{auth_port}/api/threads/{tid}/linked-files/raw"
            ))
            .query(&[("path", external)])
            .send()
            .await
