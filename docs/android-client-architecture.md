...
- `api`: REST transport, connection modes, auth/session DTOs, relay portal/device DTOs, websocket URL derivation, first-pass websocket event stream, thread detail fetch, workspace tree/file-preview fetch, runtime config and workspace settings, prompt send, interrupt, thread settings, goal update, compact, pending request response, rename, and delete methods.
...
| `AppShellSettingsDialog` workspace defaults section | `AppShellSettingsPanel` + `SupervisorApiClient` | Native backend settings section loads runtime config from `GET /api/config/runtime`, loads workspace defaults from `GET /api/config/workspace-settings`, shows runtime mode/host/port and workspace root, and saves dev home/default backend through `PATCH /api/config/workspace-settings`. |
...
- Full app shell destination behavior is still open: the connected home now switches between Workspaces/Threads and uses the supervisor snapshot, but workspace drill-in, runtime controls/config archives, and richer thread-list filtering are not yet complete.
