...
- Remaining real composer actions behind the current native sample menus: skills, MCP, hooks, attachments, and fuller shell controls. Model updates, reasoning effort updates, fast mode, plan mode, goal update, compact, and fork now call real thread APIs from the detail route.
