...
The first APK now has a first-run supervisor connection setup path plus the first real thread-detail REST path. The setup path supports local, server, and relay connection modes, persists the selected endpoint, logs in to server and relay auth endpoints, and follows the relay account/device model rather than QR pairing. In relay mode the Android client can load `/relay/portal`, show registered backend devices with online/last-heartbeat status, create a new relay device, and display the one-time `rcd_...` token plus a `remote-codex relay-supervisor` command for the private backend. After a connection is saved, the app reads a backend home snapshot from `/api/workspaces` and `/api/threads` using direct or relay-forwarded paths. Thread rows now open a real `/api/threads/:id` detail screen, project server turns, pending requests, and answered request notes into the native timeline, load workspace trees through `/api/workspaces/:id/files/tree`, load a bounded file preview through `/api/workspaces/:id/files/preview`, read shell state through `/api/threads/:id/shell`, create shells through `POST /api/threads/:id/shell`, terminate shells through `POST /api/shells/:id/terminate`, send prompts through `/api/threads/:id/prompt`, interrupt active turns through `/api/threads/:id/interrupt`, update thread model/reasoning/fast/plan settings through `PATCH /api/threads/:id/settings`, update goals through `PATCH /api/threads/:id/goal`, compact context through `POST /api/threads/:id/compact`, answer pending requests through `/api/threads/:id/requests/:requestId/respond`, rename threads through `PATCH /api/threads/:id`, and delete threads through `DELETE /api/threads/:id`. The detail route also opens `/ws` or `/relay/devices/:deviceId/ws`, listens for matching `thread.*` envelopes, and refreshes the aggregate detail projection when live thread events arrive. The sample thread detail surface remains as a fallback and component preview while a full event reducer, PTY shell input/output streaming, and remaining workspace file actions are completed.
...
| `GraphWorkspaceExplorer.tsx` | `WorkspacePanel.kt` | Workspace file tree from `/api/workspaces/:id/files/tree`, selected row state for the loaded preview file, root label, row-level download glyph action, refresh/garbage action glyph chips, workspace summary strip, empty workspace state, and Web-like native folder/file/artifact/event glyphs. |
...
| `GraphWorkspacePreviewPane.tsx` | `WorkspacePanel.kt` | Code preview pane backed by `/api/workspaces/:id/files/preview` for the first file in the tree, with metadata bar, native copy/open action glyph chips, truncated preview warning chip, load-more footer affordance, and scrollable monospaced content. |
...
| `ThreadShellPanel.tsx` | `ShellPanel.kt` | Terminal shell frame now backed by real REST shell state on the thread detail route, with Web-like native connection/terminate glyph controls, active process bar, live process count, mobile process drawer with no-live-process empty state, real new-shell action, real terminate action, floating shell toolbox trigger, feedback pill, disabled-aware tone-matched control chips, state output, and command bar with disconnected/running disabled states. PTY input/output streaming still needs websocket wiring. |
...
- Remaining real composer actions behind the current native sample menus: fork, skills, MCP, hooks, attachments, and shell controls. Model updates, reasoning effort updates, fast mode, plan mode, goal update, and compact now call real thread APIs from the detail route.
...
- Real workspace actions behind the current native action chips: refresh, copy, open, load more, upload, download, and file mutation. Initial workspace tree and first-file preview fetches now call the backend from the thread detail route.
...
- Remaining shell adapter actions behind the current native shell controls: attach, rename, split pane, copy visible output, PTY input/output streaming, and control sequences. REST shell state, create, and terminate now call the backend from the detail route.
