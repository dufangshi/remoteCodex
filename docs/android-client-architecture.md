...
| `GraphChatMessageFrame.tsx` | `ThreadTimelineComponents.kt` + `StatusBadge.kt` + `ThreadPresentation.kt` | User/assistant message surfaces, transparent assistant frame treatment, sender label, Web-compatible message status classification, running-dot status badges, time treatment, and assistant reply copy action. |
...
| `graphChatToolBlocks.ts` | `GraphChatToolBlocks.kt` + `RichMessageContent.kt` + `GraphAccordion.kt` | Native preprocessing for `tool-call`, `tool-result`, and merged tool blocks, with accordion disclosure, structured tool title, call id, typed key/value parameter and result rows, raw output blocks for `stdout`/`stderr`/`result`, status, and copy rendering inside rich messages. |
