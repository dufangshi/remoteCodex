...
- `ShellPanel.kt` (retained behind a disabled feature flag)
- `RemoteCodexTheme.kt`
...
| `GraphChatShellLayout.tsx` | `GraphChatShellLayout.kt` + `ThreadDetailPreviewScreen.kt` | Native root, frame, main panel, topbar shell, split region, mobile scrim, and rooms rail shell wrap the preview screen. The shell/terminal panel itself is retained but feature-gated off on Android. |
...
| `ThreadShellPanel.tsx` | `ShellPanel.kt` | Shell UI and adapters are retained in code but disabled on Android by `AndroidFeatureFlags.ShellEnabled = false`. The active mobile plan does not continue terminal parity work unless shell access is explicitly re-enabled later. |
...
- Shell parity is intentionally out of active scope for Android. The existing shell code is retained but hidden behind `AndroidFeatureFlags.ShellEnabled = false`, so the app does not expose shell navigation or call backend shell APIs by default.
