...
- Remaining shell adapter actions behind the current native shell controls: rename, split pane, copy visible output, resize UI, clear UI, richer terminal escape-sequence handling, and full PTY parity. REST shell state/create/terminate plus websocket attach/input/Ctrl-C/output now call the backend from the detail route.
