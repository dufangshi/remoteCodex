...
- `api`: REST transport, connection modes, auth/session DTOs, relay portal/device DTOs, websocket URL derivation, first-pass websocket event stream, thread detail fetch, workspace tree/file-preview fetch, prompt send, interrupt, thread settings, goal update, compact, pending request response, rename, and delete methods.
...
| `AppShellSettingsDialog` plugin section | `AppShellSettingsPanel` + `SupervisorApiClient.importPlugin` + `AppShellPreview` | Native plugin/renderers settings surface showing enabled state with shared selection glyphs, renderer status with graph badges, plugin source badges, capabilities, and a plugin manifest JSON import form wired to `POST /api/plugins/import` in connected mode with preview-local validation fallback. |
...
- Full app shell destination behavior is still open: the connected home now switches between Workspaces/Threads and uses the supervisor snapshot, but workspace drill-in, broader plugin management actions, backend settings, and richer thread-list filtering are not yet complete.
