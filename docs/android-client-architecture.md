...
| `graphChatToolBlocks.ts` | `GraphChatToolBlocks.kt` + `RichMessageContent.kt` + `GraphAccordion.kt` | Native preprocessing for `tool-call`, `tool-result`, and merged tool blocks, with accordion disclosure, structured tool title, call id, parameter, result, status, and copy rendering inside rich messages. |
...
- Full graph-chat tool block parity: robust JSON parsing, richer value-specific formatting, streaming result reconciliation, and plugin-aware renderers. Current Android support is lightweight and preview-oriented, with native copy actions and structured parameter/result sections.
