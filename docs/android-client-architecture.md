...
The active APK has a first-run supervisor setup path plus a native app shell. The setup path supports saved Local/Intranet, Server, and Relay devices; persists names, endpoints, auth/session state, selected relay devices, and per-device last route; logs in to server and relay auth endpoints; and follows the relay account/device model rather than QR pairing. In relay mode the Android client can load `/relay/portal`, show registered backend devices with online/last-heartbeat status, create a new relay device, display the one-time `rcd_...` token plus a `remote-codex relay-supervisor` command for the private backend, refresh device status, auto-select a valid backend with online devices preferred, and revoke stale backend devices through `DELETE /relay/devices/:deviceId`.

After a connection is saved, the native shell reads a backend home snapshot from `/api/workspaces` and `/api/threads` using direct or relay-forwarded paths. Workspace rows can call `POST /api/workspaces/:id/open`, `POST /api/workspaces/:id/favorite`, `PATCH /api/workspaces/:id`, `DELETE /api/workspaces/:id`, and `POST /api/threads/start`, then refresh the home snapshot or open the newly started thread. Destructive workspace/thread/device actions must keep an explicit confirmation step.

Thread detail is no longer an independent native Compose product surface. Production and default debug builds route threads through `ThreadDetailWebViewScreen.kt`, which loads the bundled Android thread-web assets from `app/src/main/assets/thread-ui` with `WebViewAssetLoader` and hosts the shared `@remote-codex/thread-ui` experience. The WebView host injects connection configuration, current thread id, fixture mode, theme mode, and native callbacks for opening threads/workspaces/devices, closing thread detail, choosing files, saving/sharing exports, copying text, and forwarding authenticated REST requests. REST and WebSocket behavior should be implemented in the shared thread UI whenever possible; Android should only own the native shell, device management, storage/share/file-picker bridges, and platform lifecycle.

The old native Compose thread timeline, composer, workspace explorer, reducers, and sample thread detail code remain in the source tree as a temporary debugging and component-preview fallback only. `AndroidFeatureFlags.NativeThreadDetailFallbackEnabled` is backed by `BuildConfig.REMOTE_CODEX_NATIVE_THREAD_DETAIL_FALLBACK`; production and default debug builds set it to `false`, and it should only be enabled with `-PremoteCodex.nativeThreadFallback=true` while debugging an Android-native parity gap. `AndroidFeatureFlags.ShellEnabled` also remains `false`; shell/terminal entry points and backend shell calls are disabled by default and are not part of the active Android parity plan.
...
Android now aligns thread detail by hosting the same shared thread UI that powers the web/mobile experience instead of reimplementing the thread surface in Compose.

- Shared source: `@remote-codex/thread-ui`.
- Android bundle source: `apps/android/thread-web`.
- Android asset output: `apps/android/app/src/main/assets/thread-ui`.
- Production host: `ThreadDetailWebViewScreen.kt`.
- Native bridge scope: authenticated REST forwarding, file picker, Downloads/share sheet, clipboard, app navigation callbacks, theme mode, and lifecycle.
- Native app scope outside thread detail: Devices, Home, Workspace list/detail, saved-device persistence, connection settings, destructive confirmations, and route restoration.

The Android WebView shell should avoid duplicating chat, workspace explorer, viewer, settings, model picker, sandbox mode, transcript export, pending request, and streaming transcript behavior. Those behaviors should stay in `@remote-codex/thread-ui` unless they require a platform bridge.

The following Compose thread files are legacy fallback/component-preview inventory rather than the production thread route. Keep them only while the debug fallback is useful, and do not add new product behavior there unless the fallback is explicitly reactivated:
...
The sample-data thread detail screen is still available only when `REMOTE_CODEX_NATIVE_THREAD_DETAIL_FALLBACK` is explicitly enabled for debugging. The normal connected app route should open the WebView thread host.

## Legacy Native Coverage Matrix

This matrix records historical native Compose parity work and can help delete or quarantine fallback code safely. It is not the active production thread-detail implementation plan. For new thread behavior, update `@remote-codex/thread-ui`, rebuild the package and Android thread-web assets, then validate the Android WebView host contract.
...
| `ThreadDetailSurface.tsx` | `ThreadDetailWebViewScreen.kt` + bundled `apps/android/thread-web` assets | Production/default debug route hosts shared `@remote-codex/thread-ui` in a WebView. `ThreadDetailScreen.kt` and `ThreadDetailPreviewScreen.kt` are legacy fallback/component-preview code behind `REMOTE_CODEX_NATIVE_THREAD_DETAIL_FALLBACK=false` by default. |
...
| `ThreadComposer.tsx` | Shared WebView thread UI; legacy `ThreadComposer.kt` fallback | Production composer behavior, including model/reasoning/plan/sandbox controls, attachments, interrupt, prompt send, compact, fork, skills/MCP/hooks, and pending request flows, belongs to `@remote-codex/thread-ui`. `ThreadComposer.kt` remains fallback/debug inventory only. Shell prompt and shell tools are hidden while `AndroidFeatureFlags.ShellEnabled = false`. |
...
| `ThreadGraphWorkspacePanel.tsx` | Shared WebView thread UI; legacy `WorkspacePanel.kt` fallback | Production Workspace/Tool Usage/Guide/Graph/Extensions behavior belongs to `@remote-codex/thread-ui`. `WorkspacePanel.kt` is fallback/debug inventory only; the native shell may still show workspace list/detail screens outside thread detail. |
