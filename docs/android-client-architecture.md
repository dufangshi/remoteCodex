...
| `ExportTranscriptDialog.tsx` | `ThreadActionDialogs.kt` + `ExportTurnPreview` | Native export transcript dialog skeleton with latest/custom mode, PDF/HTML format controls, selected turn rows, token/price option, and icon-labeled export footer. |
...
| `Button.tsx` | `GraphUiPrimitives.kt` | Native graph button primitive with default, destructive, outline, secondary, ghost, disabled, compact size variants, and optional action glyph slots. |
