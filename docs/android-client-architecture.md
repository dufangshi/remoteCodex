...
| `ThreadComposer.tsx` | `ThreadComposer.kt` | Bottom input surface with Web-like native slash/attachment/shell glyph buttons, chat/shell view toggle preview, native slash toolbox, attachment picker, model and effort menus with shared selection glyphs, plan chip, tone-matched shell tool pills, and circular send icon action. |
