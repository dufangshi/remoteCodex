...
| `GraphChatMessageFrame.tsx` | `ThreadTimelineComponents.kt` + `StatusBadge.kt` + `ThreadPresentation.kt` + `ThreadColors.kt` | User/assistant message surfaces, transparent assistant frame treatment, Web-aligned user bubble colors, green assistant sender pill, Web-compatible message status classification, running-dot status badges, time treatment, and compact assistant reply copy action. |
