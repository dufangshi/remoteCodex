...
| `GraphChatToolCall.tsx` | `ThreadTimelineComponents.kt` + `GraphAccordion.kt` | Tool call accordion with Web-like wrench glyph, monospaced name, status badge, parameters, result block with empty-output placeholder, auto-open behavior for running/output states, and copy actions for parameters/results. |
