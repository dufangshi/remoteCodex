...
| `ExportTranscriptDialog.tsx` | `ThreadActionDialogs.kt` + `ExportTurnPreview` | Native export transcript dialog with stateful latest/custom mode, PDF/HTML format controls, selected turn rows with native selection glyphs, toggleable token/price option, format-specific guidance, and icon-labeled export footer. |
