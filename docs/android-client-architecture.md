...
| `GraphChatMessageFrame.tsx` | `ThreadTimelineComponents.kt` + `StatusBadge.kt` + `ThreadPresentation.kt` | User/assistant message surfaces, sender label, Web-compatible message status classification, running-dot status badges, time treatment, and assistant reply copy action. |
