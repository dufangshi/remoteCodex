...
The first APK now has a first-run supervisor connection setup path plus the first real thread-detail REST path. The setup path supports local, server, and relay connection modes, persists the selected endpoint, logs in to server and relay auth endpoints, and follows the relay account/device model rather than QR pairing. In relay mode the Android client can load `/relay/portal`, show registered backend devices with online/last-heartbeat status, create a new relay device, and display the one-time `rcd_...` token plus a `remote-codex relay-supervisor` command for the private backend. After a connection is saved, the app reads a backend home snapshot from `/api/workspaces` and `/api/threads` using direct or relay-forwarded paths. Thread rows now open a real `/api/threads/:id` detail screen, project server turns, pending requests, and answered request notes into the native timeline, load workspace trees through `/api/workspaces/:id/files/tree`, load a bounded file preview through `/api/workspaces/:id/files/preview`, send prompts through `/api/threads/:id/prompt`, interrupt active turns through `/api/threads/:id/interrupt`, update thread model/reasoning/fast/plan settings through `PATCH /api/threads/:id/settings`, update goals through `PATCH /api/threads/:id/goal`, compact context through `POST /api/threads/:id/compact`, answer pending requests through `/api/threads/:id/requests/:requestId/respond`, rename threads through `PATCH /api/threads/:id`, and delete threads through `DELETE /api/threads/:id`. The detail route also opens `/ws` or `/relay/devices/:deviceId/ws`, listens for matching `thread.*` envelopes, and refreshes the aggregate detail projection when live thread events arrive. The sample thread detail surface remains as a fallback and component preview while a full event reducer, shell actions, and remaining workspace file actions are completed.
