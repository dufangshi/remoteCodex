...
| `GraphChatMessageFrame.tsx` | `ThreadTimelineComponents.kt` + `StatusBadge.kt` + `ThreadPresentation.kt` + `ThreadColors.kt` | User/assistant message surfaces, transparent assistant frame treatment, Web-aligned user bubble colors, sender label, Web-compatible message status classification, running-dot status badges, time treatment, and assistant reply copy action. |
