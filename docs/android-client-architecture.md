...
| `ThreadWorkspaceLayout.tsx` | `ThreadTopBar.kt` + `ThreadRoomsPanel.kt` | Mobile topbar with Web-like app menu/settings glyph buttons, workspace/session/usage details disclosure, action/thread glyph pills, segmented Chat/Workspace/Shell navigation, and rooms drawer with Web-like thread message and new-thread glyphs. |
...
| `GraphWorkspaceExplorer.tsx` | `WorkspacePanel.kt` | Workspace file tree, selected row state, root label, refresh affordance, workspace summary strip, and Web-like native folder/file/artifact/event glyphs. |
