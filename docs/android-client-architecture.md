...
- Remaining real composer actions behind the current native sample menus: MCP config write, hook create/update editable forms, attachments. Model updates, reasoning effort updates, fast mode, plan mode, goal update, compact, fork, transcript export request, skills/MCP/hooks reads, and hook trust/untrust now call real thread APIs from the detail route.
