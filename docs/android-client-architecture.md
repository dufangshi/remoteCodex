...
The first APK now has a first-run supervisor connection setup path plus the first real thread-detail REST path. The setup path supports local, server, and relay connection modes, persists the selected endpoint, logs in to server and relay auth endpoints, and follows the relay account/device model rather than QR pairing. In relay mode the Android client can load `/relay/portal`, show registered backend devices with online/last-heartbeat status, create a new relay device, display the one-time `rcd_...` token plus a `remote-codex relay-supervisor` command for the private backend, refresh device status, auto-select a valid backend with online devices preferred, and revoke stale backend devices through `DELETE /relay/devices/:deviceId`. After a connection is saved, the app reads a backend home snapshot from `/api/workspaces` and `/api/threads` using direct or relay-forwarded paths. Workspace rows can call `POST /api/workspaces/:id/open`, `POST /api/workspaces/:id/favorite`, `PATCH /api/workspaces/:id`, `DELETE /api/workspaces/:id`, and `POST /api/threads/start`, then refresh the home snapshot or open the newly started thread. Thread rows now open a real `/api/threads/:id` detail screen, project server turns, pending requests, and answered request notes into the native timeline, load workspace trees through `/api/workspaces/:id/files/tree`, load and switch bounded file previews through `/api/workspaces/:id/files/preview`, append additional preview chunks with `offset`, open/copy raw selected files through `/api/workspaces/:id/files/raw`, download workspace files through `/api/workspaces/:id/files/download`, upload system-picked Android files through `/api/workspaces/:id/files/upload`, send prompts and multipart prompt attachments through `/api/threads/:id/prompt`, interrupt active turns through `/api/threads/:id/interrupt`, update thread model/reasoning/fast/plan settings through `PATCH /api/threads/:id/settings`, update goals through `PATCH /api/threads/:id/goal`, compact context through `POST /api/threads/:id/compact`, load fork targets through `GET /api/threads/:id/fork-turns`, fork latest or selected turns through `POST /api/threads/:id/fork`, load export turn options through `GET /api/threads/:id/export-turns`, download transcript exports through `GET /api/threads/:id/exports/pdf`, save them into Android Downloads, and open the system share sheet, load real skills/MCP/hooks panel data through `/api/threads/:id/skills`, `/api/threads/:id/mcp-servers`, and `/api/threads/:id/hooks`, trust or untrust hooks through `/api/threads/:id/hooks/trust` and `/api/threads/:id/hooks/untrust`, answer pending requests through `/api/threads/:id/requests/:requestId/respond`, rename threads through `PATCH /api/threads/:id`, and delete threads through `DELETE /api/threads/:id`. The detail route also opens `/ws` or `/relay/devices/:deviceId/ws`, parses matching `thread.*` envelopes with payloads, applies local projection updates for thread metadata, goals, turns, turn items, token usage, pending request lifecycle, and `thread.output.delta` streaming text, de-duplicates output deltas by turn/item/sequence, replaces streaming items with later materialized item events, keeps aggregate detail refresh as the fallback for complex projections, keeps timeline scroll anchored with tail-following, stable non-tail position, and a real composer Jump to latest action, loads older history through server-managed `beforeTurnId` pages while preserving current live state and caching deferred item details, preserves pending-request turn/item anchors from REST and WebSocket payloads, reconciles local request-resolved notes with later server answered-request notes, submits plan-decision choices immediately, disables duplicate request responses while the REST call is in flight, restores request controls with targeted error copy after backend failures, preserves optional future event id/cursor/sequence fields from socket envelopes, de-duplicates replayed event envelopes with stable event keys, orders turn items by server sequence hints when present, sanitizes native composer text input/paste/submit paths while keeping attachment token insertion stable, parses server thread context usage into the native context meter, applies `thread.context.updated` events locally, persists editable model/reasoning/fast/plan settings through the existing thread settings endpoint, and hardens transcript export post-flow with format-aware filename normalization, duplicate export guards, and separate saved-vs-shared status handling. The current backend still does not expose a durable replay cursor, so foreground resume and reconnect continue to reconcile through aggregate thread detail refresh. Shell code remains in the tree behind `AndroidFeatureFlags.ShellEnabled`, but Android shell entry points and backend shell calls are disabled by default for safety and are no longer part of the active parity plan. The sample thread detail surface remains as a fallback and component preview while deletion navigation hardening continues.

The next connection-flow refactor is specified in
[Android Connection Flow And State Restoration](android-connection-flow.md).
That document is authoritative for the planned mode-selection, relay
login/register, relay device management, connection settings, and app relaunch
restoration behavior. In particular, relay account state and selected relay
device state must be independent, and the app must restore the last thread or
workspace for the selected connection when it restarts.
...
- `api`: REST transport, connection modes, auth/session DTOs, relay portal/device DTOs, relay device create/delete methods, websocket URL derivation, first-pass websocket event stream, thread detail fetch, workspace tree/file-preview fetch, runtime config, agent runtime status, workspace settings, prompt send, interrupt, thread settings, goal update, compact, pending request response, rename, and delete methods.
...
- `thread`: thread projection state, event reducers, optimistic sends, streaming output merge, and reconnect reconciliation.
...
- `ThreadDetailSurface.tsx`: thread detail as a topbar, chat timeline, workspace switch, and composer surface.
...
- `ThreadComposer.tsx`: rounded input group, slash toolbox, attachment menu, model/effort controls, plan chip, and primary send action.
...
Android equivalents are intentionally native Compose components. Shell/terminal components are retained in the source tree only as disabled code and are not part of the active Android parity plan.
...
| --- | --- | --- |
| `AppShellNavigation.tsx` | `SupervisorHomeScreen.kt` + `AppShellPanels.kt` + `ThreadTopBar.kt` | Native connected home with switchable Workspaces/Threads destinations, supervisor summary, live workspace/thread counts, recent thread/workspace rows from the supervisor snapshot, real workspace create/open/favorite/rename/delete actions with snapshot refresh, real New Thread action that calls `/api/threads/start` and opens the created thread, settings/change-connection actions, plus the in-thread app shell drawer with shared close icon controls and active-state badges. |
...
| `AppShellSettingsDialog` workspace defaults/runtime controls section | `AppShellSettingsPanel` + `SupervisorApiClient` | Native backend settings section loads runtime config from `GET /api/config/runtime`, loads workspace defaults from `GET /api/config/workspace-settings`, lists agent runtimes from `GET /api/agent-runtimes`, shows runtime mode/host/port, workspace root, backend ready/version/install state, and saves dev home/default backend through `PATCH /api/config/workspace-settings`. |
...
| `GraphChatImageItem.tsx` | `ThreadTimelineComponents.kt` + `HistoryItemPreview` | Native image event placeholder with fixed-ratio preview block, Web-like clickable asset path row, real asset loading through `/api/threads/:id/assets/image`, image detail rendering in the shared overlay, and path copy feedback. |
| `LongTextDialog.tsx` | `LongTextDialog.kt` + `DetailPreview` | Native full-detail overlay for history actions such as Command Output, File Read Details, File Change Details, image assets, and Artifact inspection, backed by shared dialog overlay primitives. Image details decode backend bytes into a fitted native bitmap with content-type and byte-size metadata. |
...
| `ThreadComposer.tsx` | `ThreadComposer.kt` | Bottom input surface with Web-like native slash/attachment glyph buttons, editable chat prompt, real prompt submission, real stop/interrupt, real model/reasoning-effort/fast-mode/plan-mode settings updates, real goal update, real compact action, and real fork latest/selected-turn actions on the REST-backed thread detail route, empty-draft send guard, native slash toolbox with graph badge status markers, goal compose/status/error and fast-mode hint preview rows with real detail-route submit plus preview-local fallback behavior, fork turn-option status rows from `/fork-turns`, real skills list rows from `/skills`, real MCP server/tool/status rows from `/mcp-servers`, real hooks rows/warnings/errors from `/hooks`, real hook trust/untrust actions, compact attachment action menu with native photo/file glyph rows, system file picker insertion, multipart backend upload, chip removal, compact Web-like model and effort menus with shared selection glyphs plus preview-local selection/default-effort feedback, fast-mode-disabled model/effort controls, model-panel disabled reason notice and empty runtime fallback, Web-matched jump-to-latest badge with local follow-state update, and circular send icon action. Shell prompt and shell tools are hidden on Android while `AndroidFeatureFlags.ShellEnabled = false`. |
...
3. A5. Relay device management: completed relay device status refresh, online-preferred selection, created-token clearing when the created device is revoked, and backend-device revoke through `DELETE /relay/devices/:deviceId` with destructive confirmation.
4. B1. WebSocket event reducer: completed payload-preserving event parsing plus a pure Kotlin reducer for safe local updates to thread metadata, goals, turns, turn items, token usage, and pending request lifecycle, with full-detail refresh retained only as fallback for streaming deltas and complex projections.
5. B2. Streaming output merge: completed `thread.output.delta` local append, turn/item/sequence de-duplication, missing-turn refresh fallback, and materialized `thread.item.completed` replacement of streaming items without duplication. Reconnect recovery still falls back to aggregate detail refresh until B7 introduces a durable cursor.
6. B3. Timeline scroll behavior: completed shared `LazyListState` timeline control, tail visibility detection, automatic tail following only when already near the end, stable non-tail position for new events, and a real composer Jump to latest action wired to animated timeline scrolling.
7. B4. Server-managed history paging: completed `totalTurnCount` parsing, `canLoadEarlier` projection, clickable Load earlier timeline row, `beforeTurnId` page fetches, older-turn merge without overwriting live thread state, and deferred history-item detail caching.
8. B5. Request anchoring and answered-note reconciliation: completed native preservation of request `turnId`/`itemId`, answered-note `turnId`, presentation metadata for anchored request/note previews, local `thread.request.resolved` provisional answered notes, and refresh reconciliation that keeps provisional notes until the server aggregate returns the durable answered-request note.
9. B6. Pending request lifecycle: completed WebSocket-created request insertion through the reducer, immediate plan-decision option submit behavior, request-level optimistic busy state with selected-option labels, duplicate-submit guards while a response is in flight, and targeted backend error recovery that re-enables the request after failure.
10. B7. History ordering and event cursor: completed Android parsing for optional future `eventId`/`cursor`/envelope `sequence` fields, local stable event-key replay de-duplication, `lastEventCursor` tracking when a cursor exists, server `ThreadTurnItem.sequence` preservation, and sequence-aware turn-item ordering. The current supervisor API does not yet publish a durable replay cursor, so reconnect recovery still uses aggregate detail refresh until that backend contract exists.
11. C1. Composer rich text input: completed native text sanitization for input, paste-derived text, attachment insertion, and submit paths; normalized CRLF/control characters/NBSP/line-end whitespace; preserved tested file/photo attachment token insertion, duplicate suffixes, dropped-file ordering, and system-picker insertion rules without adding a heavyweight rich text editor.
12. C2. Context usage and slider persistence: completed backend-driven thread context usage parsing, native composer context meter projection from `thread.contextUsage`, local `thread.context.updated` reducer updates, unavailable-state handling when the backend has no context window, and settings persistence through the existing `PATCH /api/threads/:id/settings` model/reasoning/fast/plan flow.
13. C5. Export post-flow hardening: completed format-aware PDF/HTML filename normalization for Android Downloads, path/invalid-character/blank/long-name cleanup, duplicate export request guards while an export is active, saved-vs-shared status messages, and recovery that keeps a successfully saved export even if Android share handoff fails.
14. C6. Thread deletion navigation refresh: completed a detail-to-host deletion callback so `DELETE /api/threads/:id` success refreshes the home snapshot and routes back to Home from the activity layer, while failed deletes keep the user on the thread detail with retryable dialog error state and duplicate submits blocked by the existing thread action busy guard.
15. E1. Full Markdown/GFM parity: completed the native safe GFM subset for current thread messages, including nested list continuation preservation, task list state, escaped-pipe table cells, short-row table padding, setext headings, tilde/backtick code fences, display math blocks, and non-executing HTML fallback blocks rendered as copyable code-style content.
16. E2. KaTeX-quality math rendering: completed a native math presentation layer that replaces plain monospaced fallback with structured token layout for inline and display math, including superscript/subscript baseline shifts, `\frac{}` and `\sqrt{}` normalization, common LaTeX symbol/operator normalization, horizontal scrolling, and copy of the original expression. This is still an offline Compose renderer rather than a full TeX engine or WebView-backed KaTeX runtime.
17. E3. Markdown image loading: completed safe native inline markdown image loading for thread-relative asset paths. Assistant message markdown images now flow through an optional timeline image resolver, fetch `/api/threads/:id/assets/image` through the active direct/relay client, decode supported image bytes into Compose bitmaps, keep labels and filenames visible, and fall back to the stable placeholder for remote URLs, data/file URLs, absolute paths, traversal paths, decode failures, or preview surfaces without a resolver.
18. F1. Tool block JSON formatting: completed robust nested JSON display for tool parameters/results with order-preserving pretty formatting, structured object/array block rendering, per-block copy actions that copy the raw value rather than the pretty display text, and fallback parsing through `org.json` for valid JSON that the lightweight formatter cannot preserve.
19. F2. Streaming tool result reconciliation: completed Android-side reconciliation between streaming markdown tool blocks and materialized tool history items by preserving `callId`/`toolName` from REST/socket turn items, projecting them into history previews, and suppressing duplicate materialized tool cards when the assistant message still contains the matching streaming `tool-call` block. The reducer still keeps item facts intact and continues to upsert materialized events by item id.
20. F4. History item detail content types: completed richer deferred-detail plumbing by parsing optional backend `contentType`, `sourcePath`, and `assetPath` fields, building typed `DetailPreview` values, inferring JSON/Markdown/image-reference/plain content types when the backend omits them, rendering JSON detail with pretty formatting, rendering Markdown detail through the native rich message renderer, and preserving image/file source paths for drill-in dialogs.
21. F5. Broader clipboard affordances: completed broader native copy coverage for user messages and deferred/detail dialogs in addition to the existing assistant reply, reasoning, history item, tool block, code block, image path, top-bar id/session, skill invoke, and workspace/file copy actions. Detail dialogs now expose a direct Copy action for text and loaded image metadata, and user messages expose a stable Copy action in their footer.
