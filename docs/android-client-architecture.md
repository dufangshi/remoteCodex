...
- `api`: REST transport, connection modes, auth/session DTOs, relay portal/device DTOs, websocket URL derivation, first-pass websocket event stream, thread detail fetch, prompt send, interrupt, thread settings, pending request response, rename, and delete methods.
...
- Remaining real composer actions behind the current native sample menus: goal update, fork, skills, MCP, hooks, attachments, compact, and shell controls. Model updates, reasoning effort updates, fast mode, and plan mode now call the real thread settings API from the detail route.
