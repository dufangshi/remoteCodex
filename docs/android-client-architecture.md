...
- Real workspace actions behind the current native action chips: refresh and file mutation. Workspace tree, file selection, load-more preview, raw open/copy, file download, and an Android note upload now call the backend from the thread detail route. Full system file-picker upload is still pending.
