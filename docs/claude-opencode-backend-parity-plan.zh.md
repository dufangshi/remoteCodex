...
- [ ] Android：在本机临时禁用某个 runtime 后显示灰态和安装按钮，安装后恢复，更新后保持可用。
...
- [ ] 未安装 runtime 的灰态、安装按钮、安装后恢复，还需在 iOS/Android 各跑一次真实 E2E；Web 已由 `runtime-install-availability.spec.ts` 覆盖。
