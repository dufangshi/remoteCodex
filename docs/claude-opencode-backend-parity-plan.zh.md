...
- [x] Completion audit 2026-07-03：临时 local supervisor `127.0.0.1:9060` 以 `REMOTE_CODEX_ENABLED_AGENT_PROVIDERS=codex,claude,opencode` 启动后，`/api/agent-runtimes` 返回 `codex`、`claude`、`opencode` 均 `enabled=true` 且 `installed=true`；Claude 版本 `2.1.197 (Claude Code) (SDK 0.3.198)`，OpenCode 版本 `1.17.11 (SDK 1.17.13)`。
- [x] Completion audit 2026-07-03：同一 supervisor 上 `/api/agent-runtimes/claude/models` 返回 `sonnet`、`sonnet-1m`、`opus`、`haiku`，`/api/agent-runtimes/opencode/models` 返回 5 个模型，默认 `opencode/mimo-v2.5-free`。
- [x] Completion audit 2026-07-03：真实 Claude haiku API smoke 通过，thread `53a53e3d-d9c9-4222-93e6-8220e423e4c9`，marker `GOAL_AUDIT_CLAUDE_OK_19F28A277FD`；真实 OpenCode API smoke 通过，thread `2950e554-dc2d-409c-89bc-9c61b5a307bd`，model `opencode/mimo-v2.5-free`，marker `GOAL_AUDIT_OPENCODE_OK_19F28A2E90F`。
