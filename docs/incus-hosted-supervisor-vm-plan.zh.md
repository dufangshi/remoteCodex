...
- [ ] 重新做宿主只读体检和容量确认。
- [ ] 安装 Incus 6.0 LTS/QEMU，初始化专用 project/network/storage/profile。
- [ ] 部署 host-agent，仅允许本机/relay bridge 访问。
- [ ] 创建最小 Ubuntu VM。
- [ ] 写入 marker → graceful stop → start → marker 仍存在。
- [ ] 创建 snapshot → 修改文件 → restore → 验证回滚。
- [ ] 验证宿主其他 Docker services 未受网络/资源影响。
...
- [ ] 构建版本化 golden image。
- [ ] 自动创建 `remote-codex` 用户和目录权限。
- [ ] 安装固定版本 Codex CLI 与 remote-codex。
- [ ] 写入 `config.toml`；按选定模式生成 `auth.json` 或完成 access-token login。
- [ ] 安装/启用 supervisor systemd unit。
- [ ] 验证 Codex login、CLI smoke、SQLite、workspace、重启持久化。
- [ ] 验证日志/operation response 不含 secret。

验收：单独通过 host-agent 创建的 VM 可作为手工 relay device 上线并完成一次真实 Codex turn。

## Phase 5：Relay 数据模型与 create saga

- [ ] 添加 hosted sandbox/operation 表和迁移测试。
- [ ] 实现 admin create/list/detail/start/stop/snapshot/delete/rotate API。
- [ ] 事务创建 relay device + requested record。
- [ ] 实现异步 operation reconciliation 和补偿逻辑。
- [ ] provider timeout/retry/restart 后恢复测试。
- [ ] Hosted delete 与普通 device delete 分流。

验收：API fake-provider E2E 覆盖成功、失败、重试、删除和 relay restart。

## Phase 6：Wake、turn-aware idle 和安全 stop

- [ ] 定义并实现向后兼容 activity envelope。
- [ ] 记录有效用户活动，排除 polling/heartbeat。
- [ ] active turn 期间禁止 auto-stop。
- [ ] turn terminal 后 10 分钟 timer，带 generation 防误杀。
- [ ] supervisor drain/shutdown/SQLite close/checkpoint。
- [ ] stopped device 首次访问触发 wake，UI 可观察进度。
- [ ] relay/host-agent 重启后的 reconciliation。

验收：长 turn 超过 10 分钟仍持续运行；turn 完成后 10 分钟才关机；wake 后数据不丢。

## Phase 7：Relay Admin 和用户 UI

- [ ] Capability/degraded card。
- [ ] Create Hosted VM wizard。
- [ ] Operation progress/detail。
- [ ] Hosted VM 列表和管理动作。
- [ ] 普通 device 行保持兼容。
- [ ] 用户端 Stopped/Starting/Online/Error 状态。
- [ ] responsive/accessibility/error/secret-field 测试。
- [ ] 确认 Android/iOS DTO optional compatibility；需要时单独排移动端 phase。

验收：host-agent down 时 UI 只有 Hosted 操作 disabled，admin 其他面板和 portal 正常。

## Phase 8：独立部署 workflow

- [ ] host-agent GitHub Actions build/test/deploy/rollback。
- [ ] relay optional env 和 capability deploy evidence。
- [ ] 两个 workflow 无硬依赖。
- [ ] secrets 只存在 GitHub Secrets/宿主 secret store。
- [ ] 部署日志 secret scan。
