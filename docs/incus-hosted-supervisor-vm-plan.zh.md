...
> 状态：Phase 0-10 的初始限量上线范围已实现并部署，真实模型、turn-aware idle、Admin UI、故障注入、orphan reconciliation、off-host round-trip restore 和 v4 空线程重启均已完成生产验证。尚未闭合的外部 gate 是整机维护窗口 reboot、功能分支合入默认分支后的独立 host-agent GitHub Actions 首跑，以及扩大 1-running 配额前的独立 restore host/容量数据。
>
...
- 独立 workflow 的 YAML、shell、本地 build/test、手工同构部署和上面的失败隔离已验证；因 workflow 文件尚未存在于 GitHub 默认分支，GitHub 不会注册它，仍需合入 `main` 后取得首次独立 deploy/rollback run 证据。
- 对本分支 4 次成功 Relay Deploy run（`29137721632`、`29132577362`、`29132252329`、`29132029058`）做真实模型 key 精确扫描，匹配数均为 0；tracked git、生产 relay logs 和 host-agent journal 的匹配数也均为 0。host-agent workflow 仍需合入默认分支后取得首次独立 deploy run 证据。
...
- 最终清理后 Hosted sandbox 为 0、Incus project 无实例、credential store 文件数为 0、inventory audit healthy、relay `/healthz=200`。未重启整台多服务生产宿主；该项保留为维护窗口验收，不在本轮静默执行。
