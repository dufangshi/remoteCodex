...
> 状态：Phase 0-10 的初始限量上线范围已全部实现、合入 `main` 并完成生产验收，包括真实模型、turn-aware idle、Admin UI、故障注入、orphan reconciliation、off-host round-trip restore、独立 workflows、整机 reboot 和 v4 空线程重启。生产当前保留一台归属 `alwyn` 的 v4 Hosted VM；跨宿主 DR 和扩大 1-running 配额仍作为后续扩容 gate，不阻塞本轮限量上线完成。
>
...
- 本 Phase 的最终验收已由 Phase 9 的真实限额中转 key E2E 关闭：凭证经 relay create body → host-agent encrypted credential ref → guest stdin，device online 后完成真实 Codex turn；没有复用管理员个人 `auth.json`。
...
- 生产宿主已做无副作用 deploy failure injection：向 installer 提供不存在的 artifact，安装在任何写操作前以 exit 1 失败；旧 host-agent 保持 active、Incus active、relay `/healthz=200`，临时普通用户 device create/delete 均为 200，随后用户清理完成。
- 生产候选与正式 relay 均做了 host-agent outage：agent inactive 时 relay `/healthz=200`、Hosted capability `available=false`/`hosted_provider_unreachable`，普通用户 `POST /relay/devices=200`，已在线 Hosted tunnel 不掉线；agent 恢复后 capability 自动回到 true。
- 使用假 key 对 host-agent audit/operation/credential store、relay SQLite volume、relay logs、host-agent journal 和 relay image env 做精确 secret scan，所有宿主/relay 匹配数均为 0。key 只按设计存在于临时 guest 的 0600 `auth.json`，随后随 Hosted delete 销毁。
- PR #2 合入 `main` 后，独立 host-agent run `29142190733` 首次真实暴露 sibling `remote-codex-thread-ui` 未 checkout；commit `ff8a97c4` 在 install 前复用现有 deploy-key checkout helper，随后 main run `29142244643` 的 checkout、28 tests、build、artifact upload、install/ready 与 rollback wrapper 全部成功。
- main Relay run `29142244620` 的 image build/push 与 SSH deploy 成功，生产容器 image 为 `ff8a97c4...`；失败/成功 host-agent run 与两次 main Relay run 的真实模型 key 精确日志扫描均为 0 matches。

## Phase 9：完整真实 E2E

- [x] Admin UI 创建 Hosted VM 并分配给测试用户。
- [x] VM provision、Codex login、supervisor 自动上线。
- [x] 测试用户经 relay device API 创建 workspace/thread。
- [x] 提交带顺序 marker 和文件写入的真实 Codex turn。
- [x] turn 活跃时等待超过 10 分钟，VM 不停止。
- [x] turn 完成后 10 分钟无用户操作，VM graceful stop。
- [x] 用户再次打开 device，自动 wake。
- [x] workspace 文件、supervisor thread、`.codex` session/state 保持。
- [x] restart relay 后复验。
- [x] restart host-agent 后复验。
- [x] restart 宿主后复验。
- [x] host-agent/Incus 故障注入，普通 relay device 完整回归。

验收 sentinel 建议：`INCUS_HOSTED_RELAY_E2E_OK`，并保存 VM state、relay online/offline、marker 文件和 UI 截图证据。

### Phase 9 实施记录（2026-07-10）

- 生产 relay 以保留旧 container 的自动回滚方式切到 Incus provider；切换前后既有 device 均为 8 条，relay healthy，host-agent 仅绑定专用 `172.30.0.1:8801` bridge。
- 发布不可变 `remote-codex-ubuntu-24.04-v2`。v2 支持从当前分支 tarball 构建，确保 guest 实际包含 `relay.activity` 和 graceful shutdown 代码；固定 Codex CLI 0.144.1、Node 22.17.0。构建脚本已强制非交互 stdin。
- 真实服务器暴露并修复两项竞态：Incus ACPI stop 在该 image/host 上不触发 guest shutdown，现先固定执行 guest `systemctl poweroff`、等待 graceful transition，最终才 force；VM Running 早于 incus-agent ready，provision 现先重试固定 `incus exec true`。v2 stop 实测 1 秒。
- Admin API 创建临时用户和 Hosted sandbox 成功：create 202、operation succeeded、VM Running、supervisor tunnel online。用户创建 workspace、marker 文件和 Codex thread 均为 200；guest SQLite 为 167936 bytes、auth mode 为 apikey。
- sentinel `INCUS_HOSTED_RELAY_E2E_OK` 经手工 stop、自动 wake、relay restart、host-agent restart、snapshot、再次 wake 后保持；thread list 仍包含原 ID，`.codex/auth.json` 和 SQLite 均存在。
- 最后一次有效用户活动后以真实 600 秒配置只做 admin GET polling；deadline 未漂移、前 543 秒 VM 未提前停止，到 deadline 后 relay 状态 stopped、Incus VM STOPPED。
- host-agent inactive 时生产 relay health 200、Hosted capability false、普通 device create 200、在线 Hosted tunnel 保持 online；恢复后 capability true。
- 生产 Admin UI 已通过真实浏览器登录和点击验证：Hosted VMs 显示 Available、`0/1 running · 0/4 total`、用户/规格/API key 表单和 10 分钟 idle 文案；host-agent inactive 时显示 Unavailable、Create disabled，同时 Users 页面仍正常；agent 恢复后 Check host 回到 Available。
- E2E 结束后 Hosted delete 202 → detail 404，VM/snapshot、credential、device、临时用户和候选 volume 全部清理；credential store 文件数为 0。
- 使用真实限额中转 key 和结构化配置完成模型 E2E：`gpt-5.6-sol` 返回精确 sentinel `REAL_INCUS_CODEX_TURN_OK`，并写入三行顺序 marker `REAL_STEP_1/2/DONE`。
- 真实长 turn 在 guest 内执行 660 秒命令；第 601 秒和第 661 秒均为 `activeTurnCount=1`、`idleDeadline=null`、Incus RUNNING，terminal 后返回 `LONG_TURN_E2E_OK`，第 9 分钟仍 RUNNING，第 10 分钟 relay/Incus 均为 Stopped。
- wake 后两个 sentinel、workspace marker、`.codex` 与 supervisor SQLite 均保留；随后分别 restart host-agent 和 relay，设备恢复 online，历史和文件继续可读。
- 真实 Admin UI 已提交 `UI Hosted VM E2E`，状态从 Creating 变为 Online，显示 10 分钟 idle deadline；随后同一 UI 发起并确认 Delete，detail 变为 404。
- 生产复验暴露并修复“从未发送首条 prompt 的 Codex 空线程在 supervisor 重启后无法 resume”：仅对 supervisor 创建、零本地 turn 且 provider 明确返回 `thread not loaded/no rollout found` 的 Codex 线程重建 provider session；后续 prompt 回归通过。
- v4 上线后又做了专项生产 E2E：Admin API 创建 `ubuntu-24.04-v4` VM 和未 materialize 的空线程，stop/start VM 后 `resume=200`，首条真实模型 prompt `=200`，精确返回 `V4_EMPTY_THREAD_RESTART_OK`；随后删除 sandbox/user，inventory 回到 healthy。
- 获得明确维护授权后执行整机 reboot：boot ID 从 `1b1d78c9-fa4b-4403-ac11-237acd45c090` 变为 `56dec932-6280-465a-bf53-0e6d419f45a3`。重启前后 23/23 `unless-stopped` containers 的名称 SHA-256 同为 `bc6bfe018603aa35ad676ee60b7b9bec35d736eba5f0d815d76894a58a1bfab6`；Docker、Incus、network unit、host-agent 全部 active，relay/agent 200、v4 image/relay volume/KVM/iptables 持久化、audit healthy，普通 device create/delete 均为 200。
- reboot 后新建临时 v4 Hosted VM，create → online → stop/start → 空线程 resume/首条真实 prompt 均为 200，精确返回 `POST_REBOOT_V4_E2E_OK`，随后 sandbox/user 清理且 audit healthy。

## Phase 10：限量上线与运维

- [x] 仅 admin API 可创建，初始最大 1 台 running VM。
- [x] capacity dashboard、CPU/RAM/disk 告警。
- [x] VM/snapshot/secret orphan reconciliation。
- [x] backup/restore drill。
- [x] incident runbook：host-agent down、Incus down、VM boot failed、credential expired、disk full。
- [x] 已记录容量数据并保持 1-running 配额；只在后续达到扩容阈值后扩大。

验收：可以从空宿主或备份恢复一台 Hosted device，并有明确的删除/撤销/轮换流程。

### Phase 10 实施记录（2026-07-10）

- host-agent 增加串行 capacity lock，默认最多 4 台 managed VM、最多 1 台 Running VM；create/start 并发请求也必须在锁内重新读取 Incus usage，不能只依赖 UI 配额。
- `/v1/capability` 和 relay admin capability 透传 `total/running` usage 与 limit，Hosted VMs card 显示容量。上线时识别出 Phase 3/4 的两台 stopped fixture 属于已确认测试 orphan，经受限 delete API 清理后生产实测为 `0/1 running、0/4 total`。
- 新增 `docs/incus-hosted-supervisor-vm-runbook.zh.md`，覆盖 optional/fail-open 边界、健康检查、独立部署/回滚、五类 incident、安全删除、orphan audit 和 backup/restore 顺序。
- host-agent capability 新增 CPU/load、可用 RAM、Incus monitor path 磁盘指标和可配置阈值；Admin Hosted card 显示指标及 warning。生产实测为 6 CPU、约 8.2 GiB 可用 RAM、约 50 GiB 可用磁盘，无阈值告警。
- host-agent 新增 bearer-authenticated `/v1/inventory`，仅返回受管 VM status/snapshot 名、opaque credential ref/创建时间；relay 默认每 5 分钟审计其 SQLite 期望状态与 provider inventory。审计只检测，不自动删除。
- Admin UI 新增 inventory 状态、手工 audit、missing 记录告警和 orphan VM/credential 显式清理。删除前强制重新审计，资源已重新归属时返回 409。
- 生产故障注入直接制造 1 台 stopped orphan VM 和 1 个 orphan credential：audit 精确报告 `issues/1/1`，经 admin API 清理后回到 `healthy/0/0`。host-agent 停止期间 relay health 200、临时普通用户 device create/delete 200、Hosted audit 单独 unavailable；恢复后 audit 自动 healthy，所有测试资源清零。
- 发布 `remote-codex-ubuntu-24.04-v3`（x86_64，约 1.1 GiB），包含结构化 Codex provider 配置生成器；`config.toml` 字段经 relay/host-agent 双重校验，API key 仍只经 credential store 和 guest stdin，`auth.json` 保持 0600。
- 发布不可变 `remote-codex-ubuntu-24.04-v4`，在 v3 基础上加入 Codex 空线程 restart/resume 修复；host-agent 默认 imageVersion 同步切到 v4，v3 保留用于回滚而不覆盖。
- v4 生产镜像 fingerprint `4659ccd9690059821d4f4ec045ba06c46d128de9e00374bbcac6c5227e6e5ac5`，x86_64、1128.02 MiB；Relay workflow `29137721632` 成功，host-agent capability available、0 running/0 total、无 alerts。
- GitHub `REMOTE_CODEX_HOSTED_RELAY_SERVER_URL` 已配置为正式 relay origin，避免普通 relay workflow 因缺失 Hosted URL 安全降级为 disabled。
- 完成一次 off-host round-trip restore drill：graceful stop 后导出约 1.19 GB 的 Incus instance、better-sqlite3 一致性备份、对应 encrypted credential blob/master key metadata；归档 SHA-256 为 `31cfb9b5305c0faa839de0a032fef33de5072d80905d6a626afae78e6e8b58b3`。
- 归档下载到本机 Mac 后验证相同 SHA-256、SQLite hosted/device/operation 记录和 AES-GCM credential 可解密，再从该 off-host 副本回传。恢复到独立 Incus project，guest 不配置任何 NIC；启动后验证三行 ordered marker、`LONG_TURN_DONE`、0600 `auth.json`、自定义 model/base URL 和 167936-byte supervisor SQLite，随后销毁恢复 project。
- 这证明备份批次可离开宿主并重新导入，但不等于完整 DR ready：恢复计算仍在同一物理宿主的隔离 project。独立 x86_64 restore host、正式 object storage/key escrow、保留周期和 RPO/RTO 仍是扩大 1-running 配额前的上线 gate。
- reboot 前宿主约 8.2 GiB available RAM、30 GiB available disk；单台 Standard VM 运行时仍满足现有告警线。结论是本轮不扩大 `maxRunningInstances=1`，将独立 restore host、正式备份和更长容量观测作为未来扩容条件，而不是虚假勾选扩容。
- 最终保留 `Hosted Codex v4 Ready`：sandbox `2f527ca6-4d03-46c1-8c8f-5c051578b535`、device `4c96d211-db47-4676-856f-9cc022ce3226`，归属 `alwyn (yinalwyn123@gmail.com)`。创建 operation succeeded，relay online/connected，Incus RUNNING，supervisor active，`auth.json` mode 0600，Codex API-key login 有效，模型/base URL 为 `gpt-5.6-sol`/`https://sub.lnz-study.com`。
