...
- 未完成项：未提供专用真实 OpenAI Platform API key，因此没有模型回答、active turn 超过 10 分钟和 terminal turn 的实时证据；浏览器未提交 Create（实际创建由 API E2E 完成）；未重启整台多服务生产宿主。
...
- 尚未完成：专用真实 key 的模型 turn、active turn 超过 10 分钟、terminal 后 10 分钟 idle stop 的真实证据，以及异机 backup/restore drill。Phase 10 仍不能整体标记完成。
- 异机备份当前设为上线 gate，而不是用同机 snapshot 冒充完成：尚未指定 off-host object storage/备份机、加密密钥托管、保留周期、RPO/RTO 和隔离 restore host。在这些输入确定并完成一次隔离恢复前，不得扩大 1-running 配额或宣称 disaster recovery ready。
