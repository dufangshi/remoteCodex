...
当前实现会在正常 Hosted delete 中清理 VM、snapshot、credential 和 device；跨存储的定时 orphan reconciler 仍是后续工作。
...
- Phase 3 已完成同宿主 snapshot restore；异机 backup/restore drill 尚未完成，不能据此宣称灾备就绪。
