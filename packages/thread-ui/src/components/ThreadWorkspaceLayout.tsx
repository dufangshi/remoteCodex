...
export function ThreadWorkspaceLayout({
  ...
  const topbarRoomLabel =
    currentWorkspaceLabel ?? currentWorkspaceId ?? 'all';
  const topbarSessionLabel =
    sessionLabel ?? currentThreadLabel ?? currentThreadId ?? 'default_session';
  const topbarUsageLabel =
    usageLabel ?? (status?.state ? `runtime ${status.state}` : 'waiting for agent usage');
  ...
  async function handleCreateThreadFromDialog() {
    ...
    try {
      ...
      if (newThreadHref) {
        window.location.assign(buildNewThreadHrefWithTitle(title) ?? newThreadHref);
        return;
      }
  ...
  function renderSettingsDialog() {
    if (!settingsContent && !metaContent && !canUpdateThemeMode) {
      return null;
    }
      ...
      <Dialog>
        ...
        >
          <DialogHeader>
            ...
            <DialogDescription>
              Manage local display preferences for this browser.
            </DialogDescription>
          </DialogHeader>
  ...
  function renderRoomsRailContent(collapsed = false) {
    return (
      <div className="flex min-h-0 flex-1 flex-col">
        {showMobileAppMenu && appMenuButton && !collapsed ? (
          <section className="relative px-1 pb-3">
            <div>{appMenuButton}</div>
            {appNavigationMenu ? (
              <div className="absolute left-1 top-[calc(100%+0.45rem)] z-50 w-[min(18rem,calc(100vw-1rem))]">
                {appNavigationMenu}
              </div>
            ) : null}
          </section>
        ...
        <div className="thread-workspace-panel-header flex h-12 shrink-0 items-center justify-between gap-3 border-b border-[var(--theme-border)] px-3 sm:h-[60px] sm:px-4">
          ...
          <div className="flex shrink-0 items-center gap-1">
            ...
            <button
              ...
              className="thread-icon-button thread-desktop-only-inline-flex h-8 w-8 items-center justify-center rounded-full"
    ...
    <>
      ...
      >
        <GraphChatShellFrame roomsRailCollapsed={roomsRailCollapsed}>
              ...
              >
                <div className="flex min-w-0 items-center gap-3">
                  <button
                    type="button"
                    onClick={() =>
                      setRoomsRailCollapsed((current) => !current)
                    }
          ...
          <GraphChatMainShell>
              <GraphChatTopbarShell>
                <div className="thread-topbar-row flex min-h-14 items-center px-3 py-2 sm:min-h-16 sm:px-5">
                  <div className="flex w-full items-center justify-between gap-3 sm:gap-4">
                    <div className="flex min-w-0 items-center gap-2 sm:gap-3">
                      {renderMobileTopbarControls &&
                      showMobileAppMenu &&
                      appMenuButton ? (
                        <div className="thread-mobile-only-block">{appMenuButton}</div>
                      ) : renderMobileTopbarControls &&
                      ...
                      <div className="min-w-0 space-y-1">
                        {renderMobileTopbarControls ? (
                          <h1 className="thread-mobile-only-block min-w-0 truncate text-base font-semibold leading-none text-[var(--theme-fg)]">
                        ...
                        <div className="flex min-w-0 flex-col gap-1 sm:gap-1.5">
                          <button
                            ...
                            onClick={() => {
                              ...
                              void navigator.clipboard?.writeText(topbarRoomLabel);
                            }}
                          ...
                          <div
                            className="thread-topbar-meta-row flex min-w-0 max-w-full items-center gap-1 text-[11px] leading-none sm:text-xs"
                    ...
                    <div className="inline-flex shrink-0 items-center gap-2">
                      ...
                      {hasWorkspace && workspaceCollapsed ? (
                        <button
                          ...
                          className="thread-secondary-action thread-desktop-only-inline-flex h-9 items-center gap-2 rounded-lg border px-3 text-sm font-medium"
                        >
